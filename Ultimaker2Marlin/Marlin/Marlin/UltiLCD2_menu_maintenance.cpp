...
{
    #ifdef ALTER_EXTRUSION_MODE_ON_THE_FLY
        lcd_tripple_menu(PSTR("Extru-|System"), PSTR("ADVANCED"), PSTR("RETURN"));
    #else
        lcd_tripple_menu(PSTR("Bed Leveling"), PSTR("ADVANCED"), PSTR("RETURN"));
    #endif
    ...
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_MAIN(0))
          #ifdef ALTER_EXTRUSION_MODE_ON_THE_FLY
          {
            temp_extrusion_mode = extrusion_mode;
            lcd_change_to_menu(lcd_menu_maintenance_extru_system);
          }
          #else
...
static char* lcd_advanced_item(uint8_t nr)
{
    ...
    if (nr == 0)
        ...
        strcpy_P(card.longFilename, PSTR("Buildplate leveling"));
    else if (nr == 2)
        strcpy_P(card.longFilename, PSTR("LED settings"));
    else if (nr == 3 )
    {
        ...
        strcpy_P(card.longFilename, PSTR("Heatup buildplate"));
    #endif
    else if (nr == 3 + BED_MENU_OFFSET + extrusion_mode)
        strcpy_P(card.longFilename, PSTR("Home head"));
    else if (nr == 4 + BED_MENU_OFFSET + extrusion_mode)
        strcpy_P(card.longFilename, PSTR("Lower buildplate"));
    else if (nr == 5 + BED_MENU_OFFSET + extrusion_mode)
        strcpy_P(card.longFilename, PSTR("Raise buildplate"));
    else if (nr == 6 + BED_MENU_OFFSET + extrusion_mode)
        strcpy_P(card.longFilename, PSTR("Insert material"));
    else if (nr == 7 + BED_MENU_OFFSET + extrusion_mode)
    {
        if (extrusion_mode < 2)
...
static void lcd_advanced_details(uint8_t nr)
{
        ...
        //#if EXTRUDERS > 1
            ...
            {
                int_to_string(int(dsp_temperature[2]), buffer, PSTR("C/"));
                int_to_string(int(target_temperature[2]), buffer+strlen(buffer), PSTR("C"));
            }
        //#endif
        #if TEMP_SENSOR_BED != 0
            else if (nr == 3 + extrusion_mode)
          ...
          if (lcd_lib_button_pressed)
          {
              ...
              else if (IS_SELECTED_SCROLL(7 + BED_MENU_OFFSET + EXTRUDERS))
              {
                  ...
                  if (lcd_lib_button_pressed)
                  {
                      ...
                      else if (IS_SELECTED_SCROLL(4) && extrusion_mode == 2)
                      {
                          active_extruder = 1;
                          lcd_change_to_menu(lcd_menu_maintenance_advanced_heatup, 0);
                      }
                      else if (IS_SELECTED_SCROLL(5) && extrusion_mode == 3)
                      {
                          active_extruder = 2;
                          lcd_change_to_menu(lcd_menu_maintenance_advanced_heatup,0);
                      }
              #if TEMP_SENSOR_BED != 0
                      else if (IS_SELECTED_SCROLL(3 + extrusion_mode))                     // Heatup buildplate
                      {
                          enquecommand_P(PSTR("G28 Z"));
                          lcd_change_to_menu(lcd_menu_maintenance_advanced_bed_heatup, 0);
                      }
              #endif
                      else if (IS_SELECTED_SCROLL(3 + BED_MENU_OFFSET + extrusion_mode))   // Home head
                      {
                          lcd_lib_beep();
                          enquecommand_P(PSTR("G28 X Y"));
                          enquecommand_P(PSTR("M84"));        // Release motors
                      }
                      else if (IS_SELECTED_SCROLL(4 + BED_MENU_OFFSET + extrusion_mode))  // Lower bed
                      {
                          lcd_lib_beep();
                          enquecommand_P(PSTR("G28 Z"));
                          enquecommand_P(PSTR("M84"));        // Release motors
                      }
                      else if (IS_SELECTED_SCROLL(5 + BED_MENU_OFFSET + extrusion_mode))   // Raise bed
                      {
                          char buffer[32];
                          lcd_lib_beep();
                          enquecommand_P(PSTR("G28 Z"));
                      ...
                      else if (IS_SELECTED_SCROLL(6 + BED_MENU_OFFSET + extrusion_mode))   // Insert material
                      {
                          char buffer[32];
                          enquecommand_P(PSTR("G28 X Y"));
                          sprintf_P(buffer, PSTR("G1 F%i X%i Y%i"), int(homing_feedrate[X_AXIS]), int(X_MAX_LENGTH/2), 10);
                      ...
                      else if (IS_SELECTED_SCROLL(7 + BED_MENU_OFFSET + extrusion_mode))   // move material
                      {
                          set_extrude_min_temp(0);
                          active_extruder = 0;
                          target_temperature[active_extruder] = material[active_extruder].temperature;
                      ...
                      else if (IS_SELECTED_SCROLL(8 + BED_MENU_OFFSET + extrusion_mode) && extrusion_mode > 1)
                      {
                          set_extrude_min_temp(0);
                          active_extruder = 1;
                          target_temperature[active_extruder] = material[active_extruder].temperature;
...
static void lcd_menu_maintenance_extru_system_halt_for_user_action()
{
  ...
  lcd_lib_draw_string_centerP(10, PSTR("Change toolhead to"));
  if(extrusion_mode == 1)
